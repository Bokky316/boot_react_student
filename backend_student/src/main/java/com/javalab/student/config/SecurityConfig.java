...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(request -> request
                ...
                .requestMatchers("/ws/**").permitAll()  // 웹소켓 연결을 위한 여러가지 요청이 자동으로 오기 때문에 와일드 카드로 처리
                .requestMatchers("/topic/**").permitAll()  // ✅ STOMP 메시지 브로커 경로 허용
                .requestMatchers("/api/public/**").permitAll()  // ✅ Nginx 에서 사용하는 경로 허용
                ...
                .requestMatchers("/api/items/**").hasAnyRole("USER", "ADMIN") // 채팅방 생성, 채팅방 목록 조회 API는 USER, ADMIN만 접근 가능
                .requestMatchers("/api/payments/**").hasAnyRole("USER", "ADMIN") // 결제 요청, 결제 검증 API는 USER, ADMIN만 접근 가능
                .requestMatchers("/error").permitAll() // 에러 페이지는 인증 없이 접근 가능
                .requestMatchers(
                        "/images/**",
                        "/static-images/**",
                        "/css/**",
