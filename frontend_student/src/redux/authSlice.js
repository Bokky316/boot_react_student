...
export const fetchUserInfo = () => async (dispatch) => {
    try {
        const response = await fetchWithAuth(`/auth/userInfo`);    // 서버로 /api/auth/userInfo 요청을 보냄. 이 요청을 보낼때 토콘도 같이 보내서 그 토큰으로 인증된 사용자 정보를 가져옴
        if (!response.ok) {
        ...
        dispatch(setUser(userData)); // 응답 결과로 반환된 사용자 정보를 리덕스에 저장
