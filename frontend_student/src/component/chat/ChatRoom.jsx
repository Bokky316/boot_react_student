...
export default function ChatRoomList() {
    ...
    const fetchChatRooms = async () => {
        try {
            const response = await fetchWithAuth(`/chat/rooms/${user.id}`); // API 요청 실행
            if (response.ok) {
    ...
    const fetchUsers = async (query) => {
        try {
            const response = await fetchWithAuth(`/members/search?query=${query}`); // API 요청 실행
            if (response.ok) {
                const data = await response.json(); // 응답 데이터를 JSON으로 변환
    ...
    const handleCreateChatRoom = async () => {
        ...
        try {
            const response = await fetchWithAuth(`/chat/create`, {
