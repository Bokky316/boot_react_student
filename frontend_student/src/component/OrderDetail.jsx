 ...
 */
const OrderDetail = () => {
  const itemId = 2; // 하드코딩된 상품 ID (테스트용), 실제 디비에 있는 상품 ID로 변경 필요
  ...
  /**
   * 가맹점 UID 가져오기
   * - .env.development 리액트 환경설정 파일에 정의된 VITE_PORTONE_MERCHANT_ID=imp66240214 환경 변수를 가져와서 반환합니다.
   * - React에서 환경 변수는 import.meta.env 객체를 통해 접근할 수 있습니다.
   *   이때, import.meta.env.VITE_PORTONE_MERCHANT_ID 형태의 환경 변수를 .env.development 파일에서 정의하면, React가 빌드 과정에서
   *   해당 변수를 코드에 삽입해 줍니다.
